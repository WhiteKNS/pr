...
*/
            var newThing = new Thing({ country: thing.country, city: thing.city, description: thing.description, title: thing.title, lat: thing.lat, long: thing.long, length: thing.length, type: thing.type, dogAllowed: thing.dogAllowed});
            newThing.save(function (err, thing) {
                if (err){
                    res.send(500, {'message': err});
                }
                res.json({ 'message': 'Trails was successfully created!'});
            });
        ...
        updateThing: function(req,res)
        {
            ...
            var query = { _id: _id };
            Thing.update(query, {country: thing.country, city: thing.city, description: thing.description, title: thing.title, lat: thing.lat, long: thing.long, length: thing.length, type: thing.type, dogAllowed: thing.dogAllowed}, null, function (err, thing) {
                if (err){
                    res.send(500, {'message': err});
                }
